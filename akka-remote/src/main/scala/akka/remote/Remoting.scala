  ...
  val accepting: Receive = {
    ...
    case ShutdownAndFlush =>
      ...
      def shutdownAll[T](resources: IterableOnce[T])(shutdown: T => Future[Boolean]): Future[Boolean] = {
        Future
          .sequence(resources.toList.map(shutdown))
      ...
      endpoints.readOnlyEndpointFor(handle.remoteAddress) match {
        case Some((endpoint, _)) =>
          pendingReadHandoffs.get(endpoint) foreach (_.disassociate(
            "the existing readOnly association was replaced by a new incoming one",
            log
          ))
              ...
              case Some(Pass(ep, Some(uid))) =>
                if (handle.handshakeInfo.uid == uid) {
                  pendingReadHandoffs.get(ep) foreach (_.disassociate(
                    "the existing writable association was replaced by a new incoming one",
                    log
                  ))
