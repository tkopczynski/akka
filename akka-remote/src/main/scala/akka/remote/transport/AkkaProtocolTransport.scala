    ...
    case Event(InboundPayload(p), OutboundUnderlyingAssociated(statusPromise, wrappedHandle)) =>
      decodePdu(p) match {
        ...
        case Associate(handshakeInfo) =>
          ...
          goto(Open) using AssociatedWaitHandler(notifyOutboundHandler(wrappedHandle, handshakeInfo, statusPromise),
                                                 wrappedHandle,
      ...
      decodePdu(p) match {
        ...
        case Associate(info) =>
          if (!settings.RequireCookie || info.cookie == settings.SecureCookie) {
            ...
            initHeartbeatTimer()
            cancelTimer(handshakeTimerKey)
            goto(Open) using AssociatedWaitHandler(notifyInboundHandler(wrappedHandle, info, associationHandler),
                                                   wrappedHandle,
                                                   immutable.Queue.empty)
          } else {
            if (log.isDebugEnabled)
