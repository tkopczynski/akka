...
private[remote] object AkkaPduProtobufCodec extends AkkaPduCodec {
  ...
  override def constructPayload(payload: ByteString): ByteString =
    ByteString.ByteString1C(
      AkkaProtocolMessage.newBuilder().setPayload(PByteString.copyFrom(payload.asByteBuffer)).build.toByteArray
  ...
  private def constructControlMessagePdu(code: WireFormats.CommandType,
    ...
    ByteString.ByteString1C(
      AkkaProtocolMessage.newBuilder().setInstruction(controlMessageBuilder.build).build.toByteArray
    ) //Reuse Byte Array (naughty!)
  }
