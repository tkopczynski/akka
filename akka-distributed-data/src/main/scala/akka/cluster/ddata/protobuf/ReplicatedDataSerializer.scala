...
class ReplicatedDataSerializer(val system: ExtendedActorSystem)
   ...
   */
  private def getEntries[IKey, IValue, EntryBuilder <: GeneratedMessage.Builder[EntryBuilder], PEntry <: GeneratedMessage, PValue <: GeneratedMessage](
  ...
  def mapTypeFromProto[PEntry <: GeneratedMessage, A <: GeneratedMessage, B <: ReplicatedData](
      ...
      valueCreator: A => B
  )(implicit eh: ProtoMapEntryReader[PEntry, A]): Map[Any, B] = {
  ...
  def singleKeyEntryFromProto[PEntry <: GeneratedMessage, A <: GeneratedMessage](
      entryOption: Option[PEntry]
  ...
  private def ormapDeltaGroupOpsToProto(deltaGroupOps: immutable.IndexedSeq[ORMap.DeltaOp]): rd.ORMapDeltaGroup = {
    def createEntry(opType: rd.ORMapDeltaOp, u: ORSet[_], m: Map[_, _], zt: Int) = {
      if (m.size > 1 && opType != rd.ORMapDeltaOp.ORMapUpdate)
        ...
        val builder = rd.ORMapDeltaGroup.Entry
          .newBuilder()
          .setOperation(opType)
          .setUnderlying(orsetToProto(u))
          .setZeroTag(zt)
